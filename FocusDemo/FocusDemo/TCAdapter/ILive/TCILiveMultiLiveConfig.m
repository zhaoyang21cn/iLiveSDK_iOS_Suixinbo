...
@implementation TCILiveMultiLiveRoomEngine

- (void)setRuntimeConfig:(TCILiveMultiLiveConfig *)runtimeConfig
{
    if (_runtimeConfig)
    {
...
- (NSString *)interactUserRole
{
    if (_runtimeConfig)
    {
        return _runtimeConfig.interactUserRole;
    }
    else
    {
        return [super interactUserRole];
    }
...
@implementation TCILiveMultiLiveViewController
...
- (void)createRoomEngine
{
    if (_runtimeConfig)
    {
        if (!_roomEngine)
        {
            ...
            TCILiveMultiLiveRoomEngine *re = [[TCILiveMultiLiveRoomEngine alloc] initWith:(id<IMHostAble, AVUserAble>)_currentUser enableChat:_enableIM];
